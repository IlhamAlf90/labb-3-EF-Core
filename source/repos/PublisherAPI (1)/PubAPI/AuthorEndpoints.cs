...
public static class AuthorEndpoints
{
    public static void MapAuthorEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/{AutherId}", async Task<Results<Ok<Author>, NotFound>> (int authorid, PubContext db) =>
        {
        ...
        group.MapPut("/{AutherId}", async Task<Results<Ok, NotFound>> (int authorid, Author author, PubContext db) =>
        {
        ...
        group.MapDelete("/{AutherId}", async Task<Results<Ok, NotFound>> (int authorid, PubContext db) =>
        {
            var affected = await db.Authors
                .Where(model => model.AuthorId == authorid)
                .ExecuteDeleteAsync();
